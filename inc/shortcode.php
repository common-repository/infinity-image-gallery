...
class WPIG_ShortCode {
	...
	public function wpig_shortcode_init( $atts ) {
		...
		$args        = array(
			...
			'posts_per_page' => ( $this->style == 'style-1' || $this->style == 'style-2' ) ? $this->image_per_request : -1,
	...
	public function wpig_prepare_attachment_html_new( $attachments = array() ) {
		...
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $attachment ) {
				...
				$youtube_url = isset( $attachment['youtube_url'] ) ? $attachment['youtube_url'] : '';
				// if ( $attachment['youtube_url'] ) {
